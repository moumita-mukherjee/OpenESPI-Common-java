	...
	public void importData(InputStream stream, Long retailCustomerId)
		...
		while (its.hasNext()) {
			...
			if (intervalBlocks != null) {
				System.out.println(" intervalBlocks " + intervalBlocks);
				for (IntervalBlock ib : intervalBlocks) {

					String uplink = ib.getUpLink().getHref();
					String[] tokens = uplink.split("/IntervalBlock");
					String link = tokens[0];

					System.out.println(" intervalBlocks self self "
							+ ib.getSelfLink().getHref());
					System.out.println(" intervalBlocks uplink uplink "
							+ uplink);

					try {
						System.err.println(" link with " + link);
						MeterReading meterReading = meterReadingService
								.findByLink(link);

						System.err.println(" link with meterReading "
								+ meterReading);
			...
			if (usagePoint != null) {
				...
				System.out.println("getRelatedLinkHrefs tempRc tempRc "+retailCustomer);
				if (tempRc != null) {
					...
					System.out.println("getRelatedLinkHrefs retailCustomer retailCustomer "+retailCustomer);
					if (retailCustomer != null) {
						...
						for (String link : usagePoint.getRelatedLinkHrefs()) {
							System.out.println("getRelatedLinkHrefs RetailCustomer RetailCustomer "+link);
							if (link.contains("RetailCustomer")) {
								try {
									System.out.println(" RetailCustomer RetailCustomer "+link);
									retailCustomer = retailCustomerService
											.findByLink(link);
									if (retailCustomer != null) {
										usagePointService.associateByUUID(
												retailCustomer,
												usagePoint.getUUID());
										break;
									}
			...
			for (Authorization authorization : authorizationList) {
				...
				if (subscription != null) {
					...
					for (UsagePoint usagePoint : usagePointList) {
						...
						if (!(subscription.getUsagePoints()
								.contains(usagePoint))) {
