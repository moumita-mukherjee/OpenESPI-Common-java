...
public class IntervalBlockRepositoryImpl implements IntervalBlockRepository {
	...
	public List<IntervalBlock> findIntervalBlocksByPeriod(Long meterReadingId,
		...
		return (List<IntervalBlock>) this.em
				.createNamedQuery(IntervalBlock.QUERY_FIND_BY_PERIOD)
				.setParameter("meterReadingId", meterReadingId).setParameter("fromTime", fromTime).setParameter("toTime", toTime).getResultList();
	}
