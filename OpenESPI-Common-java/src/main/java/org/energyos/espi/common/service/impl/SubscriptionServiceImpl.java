...
public class SubscriptionServiceImpl implements SubscriptionService {
	...
	public Subscription createSubscription(OAuth2Authentication authentication,
		...
		subscription.setRetailCustomer(((User) authentication.getPrincipal())
				.getRetailCustomer());
		...
		System.err.println("authentication.getPrincipal() id "+((User)authentication.getPrincipal()).getId());
		System.err.println("authentication.getPrincipal() customer "+((User)authentication.getPrincipal()).getCustomerId());
		System.err.println("subscription getRetailCustomer "+subscription.getRetailCustomer());
		...
		while (it.hasNext()) {
			Long upid = it.next();
			System.err.println(" Linking....." + upid + "  " + usagePointId);
			if (usagePointId == null || usagePointId == 0
					|| usagePointId.longValue() == upid.longValue()) {
				UsagePoint usagePoint = resourceService.findById(upid,
						UsagePoint.class);
