...
public class UsagePointServiceImpl implements UsagePointService {
	...
	public List<UsagePoint> findAllByRetailCustomer(RetailCustomer customer) {
		List<UsagePoint> list = usagePointRepository
				.findAllByRetailCustomerId(customer.getId());
		// code to period load mr
		if (list != null) {
			for (UsagePoint up : list) {
				Iterator<MeterReading> it = up.getMeterReadings().iterator();
				while (it.hasNext()) {
					MeterReading mr = it.next();
					mr.getId();
					System.out.println("mr.getId() " + mr.getId());
				}
	...
	public UsagePoint findById(Long usagePointId) {
		UsagePoint up = this.usagePointRepository.findById(usagePointId);
		// code to period load mr
		Iterator<MeterReading> it = up.getMeterReadings().iterator();
		while (it.hasNext()) {
			MeterReading mr = it.next();
			mr.getId();
			System.out.println("ID ID mr.getId() " + mr.getId());
		}
		return up;
	}
